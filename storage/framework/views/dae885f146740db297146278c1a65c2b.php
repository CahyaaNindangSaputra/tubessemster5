...
<body>
        ...
        <div class="tab-content" id="v-pills-tabContent">
            ...
            <div class="tab-pane fade show active" id="nav-pos">
                ...
                <div class="row mb-5 g-4">
                    <div class="col-md-3"><div class="card summary-card bg-white p-4 h-100"><div class="summary-icon bg-primary bg-opacity-10 text-primary"><i class="bi bi-receipt"></i></div><div class="summary-title">Total Pesanan</div><div class="summary-value"><?php echo e($riwayatSelesai->count() + count($menungguBayar) + count($sedangProses)); ?></div></div></div>
                    <div class="col-md-3"><div class="card summary-card bg-white p-4 h-100"><div class="summary-icon bg-success bg-opacity-10 text-success"><i class="bi bi-cash-stack"></i></div><div class="summary-title">Omset Hari Ini</div><div class="summary-value">Rp <?php echo e(number_format($riwayatSelesai->sum('TOTAL_BAYAR'), 0, ',', '.')); ?></div></div></div>
                    <div class="col-md-3"><div class="card summary-card bg-white p-4 h-100"><div class="summary-icon bg-warning bg-opacity-10 text-warning"><i class="bi bi-fire"></i></div><div class="summary-title">Pesanan Aktif</div><div class="summary-value"><?php echo e(count($menungguBayar) + count($sedangProses)); ?></div></div></div>
                    <div class="col-md-3"><div class="card summary-card bg-white p-4 h-100"><div class="summary-icon bg-info bg-opacity-10 text-info"><i class="bi bi-grid-3x3-gap-fill"></i></div><div class="summary-title">Meja Terisi</div><div class="summary-value"><?php echo e(count($mejaTerisi)); ?> / <?php echo e(count($daftarMeja)); ?></div></div></div>
                </div>
                        ...
                        <div class="col-md-3 col-lg-2">
                            ...
                            <div class="card-meja <?php echo e($terisi ? 'meja-terisi' : 'meja-kosong'); ?> shadow-sm">
                                ...
                                <div class="card-body text-center py-4">
                                    ...
                                    <span class="badge <?php echo e($terisi ? 'bg-danger' : 'bg-success'); ?> bg-opacity-10 text-<?php echo e($terisi ? 'danger' : 'success'); ?> rounded-pill px-3 py-1 fw-bold" style="font-size: 0.75rem;"><?php echo e($terisi ? 'Terisi' : 'Tersedia'); ?></span>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <tbody id="list-menunggu-bayar">
                                        ...
                                        <tr id="row-bayar-<?php echo e($p->ID_PESANAN); ?>">
                                            ...
                                            <td class="text-center">
                                                <button class="btn btn-sm btn-success btn-modern text-white rounded-pill px-4" data-bs-toggle="modal" data-bs-target="#modalBayar<?php echo e($p->ID_PESANAN); ?>"><i class="bi bi-check-lg me-1"></i> Proses</button>
                                            </td>
                                        </tr>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?> <tr><td colspan="4" class="text-center py-5 text-muted small">Tidak ada antrean.</td></tr> <?php endif; ?>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                                    ...
                                    <tbody id="list-sedang-proses">
                                        ...
                                        <tr id="row-proses-<?php echo e($p->ID_PESANAN); ?>">
                                            ...
                                            <td id="status-badge-<?php echo e($p->ID_PESANAN); ?>">
                                                ...
                                                <?php elseif($p->STATUS_PESANAN == 'Dimasak'): ?> <span class="badge bg-info rounded-pill px-3 py-1">Siap Dimasak</span>
                                            ...
                                            <td>
                                                <div class="d-flex gap-2 align-items-center justify-content-end">
                                                    ...
                                                    <a href="<?php echo e(route('cetak.dapur', $p->ID_PESANAN)); ?>" target="_blank" class="btn btn-sm btn-dark rounded-circle text-white shadow-sm" title="Cetak Bon Dapur"><i class="bi bi-printer-fill"></i></a>
                                                </div>
                                            </td>
                                        </tr>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?> <tr><td colspan="4" class="text-center py-5 text-muted small">Dapur santai.</td></tr> <?php endif; ?>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                        ...
                        <div class="card-modern h-100 d-flex flex-column">
                            <div class="card-header-modern bg-white">
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <a href="<?php echo e(route('kasir.export', ['tanggal' => $tanggalFilter])); ?>" class="btn btn-sm btn-success rounded-pill shadow-sm px-3"><i class="bi bi-file-earmark-spreadsheet me-2"></i> Excel</a>
                                </div>
                            </div>
                            <div class="table-scroll-container border-0 rounded-0 flex-grow-1" style="max-height: 500px;">
                                <table class="table table-hover align-middle mb-0 text-center">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td class="small text-muted"><?php echo e($p->pembayaran ? substr($p->pembayaran->WAKTU_PEMBAYARAN, 11, 5) : '-'); ?></td>
                                            ...
                                            <td>
                                                <div class="d-flex justify-content-center gap-2">
                                                    <button class="btn btn-sm btn-light rounded-circle border shadow-sm" data-bs-toggle="modal" data-bs-target="#detailPesanan<?php echo e($p->ID_PESANAN); ?>"><i class="bi bi-eye"></i></button>
                                                    <a href="<?php echo e(route('cetak.struk', $p->ID_PESANAN)); ?>" target="_blank" class="btn btn-sm btn-dark rounded-circle text-white shadow-sm"><i class="bi bi-printer-fill"></i></a>
                                                </div>
                                            </td>
                                        </tr>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?> <tr><td colspan="4" class="text-center py-5 text-muted small">Belum ada data.</td></tr> <?php endif; ?>
                                    </tbody>
                                </table>
                            </div>
                            <div class="p-3 border-top bg-white mt-auto">
                                <div class="d-flex justify-content-between align-items-center bg-success-subtle p-4 rounded-4 shadow-sm border border-success-subtle">
                                    <div><small class="text-success fw-bold text-uppercase d-block mb-1">TOTAL OMSET</small><div class="badge bg-success rounded-pill px-3 py-1 fs-6"><?php echo e(\Carbon\Carbon::parse($tanggalFilter)->format('d M')); ?></div></div>
                                    <h2 class="fw-extrabold text-success mb-0 fs-3">Rp <?php echo e(number_format($riwayatSelesai->sum('TOTAL_BAYAR'), 0, ',', '.')); ?></h2>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="tab-pane fade <?php echo e($loop->first ? 'show active' : ''); ?>" id="<?php echo e($tabId); ?>">
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="ps-4"><div class="d-flex align-items-center gap-3"><?php if($m->FOTO): ?> <img src="/images/menu/<?php echo e($m->FOTO); ?>" class="img-menu shadow-sm"> <?php else: ?> <div class="img-menu bg-light small text-muted d-flex align-items-center justify-content-center">No IMG</div> <?php endif; ?> <span class="fw-bold"><?php echo e($m->NAMA_MENU); ?></span></div></td>
                                        <td>Rp <?php echo e(number_format($m->HARGA_SATUAN, 0, ',', '.')); ?></td>
                                        <td><form action="<?php echo e(route('menu.status', $m->ID_MENU)); ?>" method="POST" class="ajax-form-menu"><?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?><button class="badge border-0 <?php echo e($m->STATUS_TESEDIA == 'tersedia' ? 'bg-success' : 'bg-secondary'); ?> rounded-pill px-3 py-2 fw-bold btn-status-menu"><?php echo e(ucfirst($m->STATUS_TESEDIA)); ?></button></form></td>
                                        <td class="text-end pe-4">
                                            <div class="d-flex justify-content-end gap-2">
                                                <button class="btn btn-sm btn-light border rounded-pill px-3 fw-bold text-muted shadow-sm" data-bs-toggle="modal" data-bs-target="#editMenu<?php echo e($m->ID_MENU); ?>"><i class="bi bi-pencil-square"></i> Edit</button>
                                                <button class="btn btn-sm btn-danger bg-opacity-10 text-danger border-0 rounded-circle shadow-sm" data-bs-toggle="modal" data-bs-target="#hapusMenu<?php echo e($m->ID_MENU); ?>"><i class="bi bi-trash"></i></button>
                                            </div>
                                        </td>
                                    </tr>
                                    <div class="modal fade" id="editMenu<?php echo e($m->ID_MENU); ?>" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><div class="modal-content shadow-lg border-0"><div class="modal-header border-0 pb-0"><h5 class="modal-title fw-bold">Edit Menu</h5><button class="btn-close" data-bs-dismiss="modal"></button></div><form action="<?php echo e(route('menu.update', $m->ID_MENU)); ?>" method="POST" enctype="multipart/form-data"><?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?><div class="modal-body"><div class="mb-3"><label>Nama</label><input type="text" name="NAMA_MENU" class="form-control" value="<?php echo e($m->NAMA_MENU); ?>" required></div><div class="mb-3"><label>Harga</label><input type="number" name="HARGA_SATUAN" class="form-control" value="<?php echo e($m->HARGA_SATUAN); ?>" required></div><div class="mb-3"><label>Kategori</label><select name="ID_KATEGORI" class="form-select"><option value="K01" <?php echo e($m->ID_KATEGORI=='K01'?'selected':''); ?>>Makanan</option><option value="K02" <?php echo e($m->ID_KATEGORI=='K02'?'selected':''); ?>>Minuman</option><option value="K03" <?php echo e($m->ID_KATEGORI=='K03'?'selected':''); ?>>Snack</option></select></div><div class="mb-3"><label>Foto</label><input type="file" name="FOTO" class="form-control"></div></div><div class="modal-footer border-0"><button class="btn btn-primary w-100 rounded-pill">Simpan</button></div></form></div></div></div>
                                    <div class="modal fade" id="hapusMenu<?php echo e($m->ID_MENU); ?>" tabindex="-1"><div class="modal-dialog modal-sm modal-dialog-centered"><div class="modal-content shadow-lg border-0"><div class="modal-body text-center p-4"><div class="text-danger mb-3"><i class="bi bi-exclamation-circle" style="font-size:3rem;"></i></div><h5 class="fw-bold">Hapus?</h5><p class="small text-muted">Menu <b><?php echo e($m->NAMA_MENU); ?></b> akan hilang permanen.</p><form action="<?php echo e(route('menu.destroy', $m->ID_MENU)); ?>" method="POST"><?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?><button class="btn btn-danger w-100 rounded-pill mb-2">Ya, Hapus</button><button type="button" class="btn btn-light w-100 rounded-pill" data-bs-dismiss="modal">Batal</button></form></div></div></div></div>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </tbody>
                            </table>
                        </div>
    ...
    <?php $__currentLoopData = $semuaPesanan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="modal fade" id="detailPesanan<?php echo e($p->ID_PESANAN); ?>" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><div class="modal-content shadow-lg"><div class="modal-header bg-light border-0"><h5>Pesanan #<?php echo e($p->ID_PESANAN); ?></h5><button class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body p-0"><table class="table table-striped mb-0"><tbody><?php $__currentLoopData = $p->detail; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><tr><td class="ps-4"><?php echo e($item->menu->NAMA_MENU ?? 'Menu Dihapus'); ?></td><td class="fw-bold text-center">x<?php echo e($item->QTY); ?></td><td class="text-end pe-4"><?php echo e(number_format($item->SUBTOTAL, 0, ',', '.')); ?></td></tr><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></tbody><tfoot class="bg-white"><tr><td colspan="2" class="ps-4 fw-bold">TOTAL</td><td class="text-end pe-4 fw-bold text-primary"><?php echo e(number_format($p->TOTAL_BAYAR, 0, ',', '.')); ?></td></tr></tfoot></table></div></div></div></div>
        <div class="modal fade" id="modalBayar<?php echo e($p->ID_PESANAN); ?>" tabindex="-1"><div class="modal-dialog modal-sm modal-dialog-centered"><div class="modal-content shadow-lg border-0"><div class="modal-header bg-success text-white border-0 justify-content-center"><h5 class="fw-bold">Konfirmasi Bayar</h5></div><form action="<?php echo e(route('pesanan.bayar', $p->ID_PESANAN)); ?>" method="POST" class="ajax-form-bayar" data-id="<?php echo e($p->ID_PESANAN); ?>"><?php echo csrf_field(); ?><div class="modal-body text-center p-4"><h2 class="fw-bold"><?php echo e(number_format($p->TOTAL_BAYAR, 0, ',', '.')); ?></h2><select name="id_metode" class="form-select mt-3" required><option value="">Pilih Metode...</option><?php $__currentLoopData = $metodeBayar; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $mb): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($mb->ID_METODE); ?>"><?php echo e($mb->NAMA_METODE); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></select></div><div class="p-4 pt-0"><button class="btn btn-success w-100 rounded-pill">Terima Pembayaran</button></div></form></div></div></div>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>

    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            <?php if(session('success')): ?> Swal.fire({icon:'success', title:'Berhasil!', text:'<?php echo e(session('success')); ?>', timer:2000, showConfirmButton:false}); <?php endif; ?>
            <?php if(session('error')): ?> Swal.fire({icon:'error', title:'Gagal!', text:'<?php echo e(session('error')); ?>'}); <?php endif; ?>
