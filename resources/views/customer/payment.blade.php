    ...
    <div class="container" style="max-width: 500px;">
        <div class="card shadow-sm border-0 rounded-4 overflow-hidden">
            <div class="card-header bg-white border-0 pt-4 px-4 text-center">
                ...
                <p class="text-muted small">Pesanan #{{ $order->ID_PESANAN }}</p>
            </div>
            
            <div class="card-body p-4">
                <div class="alert alert-success text-center rounded-3 mb-4">
                    <small class="d-block text-muted">Total Tagihan</small>
                    <h2 class="fw-bold mb-0">Rp {{ number_format($order->TOTAL_BAYAR, 0, ',', '.') }}</h2>
                </div>

                <form action="{{ route('customer.payment.confirm') }}" method="POST">
                    @csrf
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-6">
                            <label class="w-100 h-100">
                                <input type="radio" name="id_metode" value="{{ $m->ID_METODE }}" class="payment-input" required>
                                <div class="payment-card p-3 text-center h-100 d-flex flex-column justify-content-center align-items-center">
                                    ...
                                    <span class="fw-bold small">{{ $m->NAMA_METODE }}</span>
                            ...
                            @if(session()->has('customer_nama'))
                                ...
                                <div class="d-flex align-items-center bg-white p-3 rounded-3 border shadow-sm">
                                    ...
                                    <div>
                                        ...
                                        <h5 class="fw-bold mb-0 text-dark">{{ session('customer_nama') }}</h5>
                                        <small class="text-muted">{{ session('customer_phone') }}</small>
                                    </div>
                                </div>
                    
                                <input type="hidden" name="nama_pelanggan" value="{{ session('customer_nama') }}">
                                <input type="hidden" name="nomor_hp" value="{{ session('customer_phone') }}">
                                
                                <div class="mt-2 text-end">
                                    <a href="{{ route('customer.logout') }}" class="text-danger small text-decoration-none fw-bold">
