...
<head>
    ...
    <meta name="csrf-token" content="{{ csrf_token() }}"> <title>Menu Riels Coffee - Meja {{ session('customer_meja') }}</title>
    ...
    <style>
        ...
        .nav-link-custom.active {
            color: #fff !important;
            background-color: transparent !important; /* Matikan background bawaan bootstrap */
        }
        /* Si Bubble Keren */
        .glider {
            position: absolute;
            ...
            border-radius: 50px;
            z-index: 1;
            transition: 0.3s cubic-bezier(0.68, -0.55, 0.265, 1.55); /* Efek membal */
        }
        ...
        .cart-bump { transform: scale(1.2); } /* Animasi saat nambah cart */
        ...
        /* ========================================== */
        /* 🎁 CSS TAMBAHAN FITUR GIVEAWAY 🎁 */
        /* ========================================== */
        .btn-giveaway {
            position: fixed;
            bottom: 90px; /* Saya naikkan dikit biar gak nabrak tombol Cart */
            right: 20px;
            width: 60px;
            height: 60px;
...
<body>
            ...
            <span class="badge bg-white text-dark rounded-pill px-3 py-2 fw-bold shadow-sm d-flex align-items-center">
                ...
                @if(session('customer_nama'))
                    ...
                    <a href="{{ route('customer.logout') }}" 
                       id="btn-reset"
                       class="btn btn-danger btn-sm rounded-circle ms-2 d-flex align-items-center justify-content-center p-0 shadow-sm" 
    ...
    <div class="container py-3 mb-5">
        @if(session('success') && !str_contains(session('success'), 'berhasil ditambahkan'))
        ...
        <div class="welcome-banner d-flex justify-content-between align-items-center">
            <div>
                <h5 class="fw-bold mb-0">
                    Halo, {{ session('customer_nama') ?? 'Pelanggan' }}! 👋
                </h5>
        ...
        <div class="nav-pills-container">
            <div class="nav nav-pills-custom" id="pills-tab" role="tablist">
                <div class="glider"></div> <button class="nav-link nav-link-custom active" data-bs-toggle="pill" data-bs-target="#makanan" onclick="moveGlider(this)">Makanan</button>
        ...
        <div class="tab-content">
            @php 
                $categories = ['makanan' => 'K01', 'minuman' => 'K02', 'snack' => 'K03']; 
                $cartCount = count((array) session('cart'));
            @endphp

            @foreach($categories as $tabId => $catId)
            <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="{{ $tabId }}">
                <div class="row g-3">
                    ...
                    <div class="col-12 text-center py-5 text-muted">
                        ...
                        <small>Menu kategori ini kosong.</small>
                    </div>
                    @endforelse
                </div>
            </div>
    ...
    <button class="btn btn-dark floating-history" data-bs-toggle="modal" data-bs-target="#modalRiwayat">
        <i class="bi bi-receipt me-1"></i> Status
    </button>
    ...
    <button class="btn-giveaway" data-bs-toggle="modal" data-bs-target="#modalGiveaway">
        <i class="bi bi-gift-fill"></i>
        <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger border border-light" style="font-size: 10px;">!</span>
    </button>

    <div class="modal fade" id="modalGiveaway" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg" style="border-radius: 20px; overflow: hidden;">
                <div class="modal-giveaway-header position-relative overflow-hidden">
                    <div class="position-absolute top-0 start-0 w-100 h-100" style="opacity: 0.1; background-image: radial-gradient(#fff 2px, transparent 2px); background-size: 20px 20px;"></div>
                    <div class="gift-icon-big">🎁</div>
                    <h3 class="fw-bold mb-1">GIVEAWAY SPESIAL!</h3>
                    <p class="mb-0 text-white-50">Edisi Khusus Pelanggan Riels Coffee</p>
                </div>
                <div class="modal-body text-center p-4">
                    <h5 class="fw-bold text-dark">Menangkan Grand Prize:</h5>
                    <h1 class="fw-bold text-primary mb-3" style="font-size: 2.2rem;">IPAD GEN 10</h1>
                    <img src="https://img.freepik.com/free-vector/realistic-tablet-device_23-2148192080.jpg?w=740" alt="Hadiah iPad" class="img-fluid rounded-4 shadow-sm mb-4" style="max-height: 160px;">
                    <div class="alert alert-warning border-0 rounded-3 text-start small">
                        <strong>📢 Cara Ikutan:</strong>
                        <ul class="mb-0 ps-3 mt-1">
                            <li>Lakukan pemesanan minimal <strong>Rp 50.000</strong>.</li>
                          
                            <li>Dapatkan kupon undian langsung!</li>
                        </ul>
                    </div>
                    <button class="btn btn-primary w-100 rounded-pill py-3 fw-bold shadow-lg mt-2" data-bs-dismiss="modal">
                        SIAP, SAYA MAU PESAN! 🚀
                    </button>
                </div>
            </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable text-dark">
            <div class="modal-content border-0 shadow-lg rounded-4">
                <div class="modal-header bg-dark text-white border-0">
                    <h6 class="modal-title fw-bold">Riwayat Pesanan Meja {{ session('customer_meja') }}</h6>
                ...
                <div class="modal-body bg-light">
                    ...
                    <div class="card border-0 shadow-sm mb-3 rounded-4 overflow-hidden">
                        <div class="card-header bg-white border-0 d-flex justify-content-between align-items-center pt-3">
                            <span class="fw-bold small">Order #{{ $p->ID_PESANAN }}</span>
                            @php
                                $statusMap = [
                                    'Proses' => ['bg-secondary', 'Sedang Diproses'],
                                    'Dimasak' => ['bg-warning text-dark', 'Sedang Dimasak'],
                                    'Siap' => ['bg-info text-white', 'Siap Diantar'],
                                    'Selesai' => ['bg-success', 'Selesai']
                                ];
                                $s = $statusMap[$p->STATUS_PESANAN] ?? ['bg-secondary', $p->STATUS_PESANAN];
                            @endphp
                            <span class="badge {{ $s[0] }} rounded-pill">{{ $s[1] }}</span>
                        </div>
                        <div class="card-body pt-0">
                            ...
                            <div class="d-flex justify-content-between fw-bold">
                                <span>Total</span>
                                <span>Rp {{ number_format($p->TOTAL_BAYAR, 0, ',', '.') }}</span>
                            </div>
                        </div>
                        <div class="card-footer bg-white border-0 pb-3 pt-0 text-center">
                            @php $isLunas = \DB::table('pembayaran')->where('ID_PESANAN', $p->ID_PESANAN)->exists(); @endphp
                            <small class="badge {{ $isLunas ? 'bg-success-subtle text-success' : 'bg-danger-subtle text-danger' }} rounded-pill px-3">
    ...
    <script>
        // 1. SCRIPT BUBBLE NAVIGATION (GLIDER)
        // Fungsi ini jalan pas halaman dimuat & pas tombol diklik
        ...
        // Jalankan saat load pertama kali (ke tab aktif)
        ...
        document.getElementById('cariMenu').addEventListener('keyup', function() {
            ...
            cards.forEach(card => {
                ...
                if(title.includes(value)) { card.style.display = ''; } 
                else { card.style.display = 'none'; }
            });
        });

        // 3. SCRIPT SWEETALERT RESET
        ...
        if (btnReset) {
            btnReset.addEventListener('click', function(e) {
                ...
                Swal.fire({
                    ...
                    text: "Nama dan keranjang belanja akan dihapus/reset.",
                    icon: 'warning',
        ...
        // 4. SCRIPT ADD TO CART TANPA REFRESH (AJAX)
        function addToCart(e, form) {
            e.preventDefault(); // Stop reload halaman
            
            const btn = form.querySelector('button');
            const originalContent = btn.innerHTML; // Simpan teks asli tombol
            ...
            fetch(form.action, {
                ...
                headers: {
                    'X-Requested-With': 'XMLHttpRequest' // Biar Laravel tau ini AJAX
                }
            })
            .then(response => {
                // Walaupun Laravel redirect back(), fetch tetap dianggap sukses
                // Kita update UI manual aja biar cepet
                ...
                cartBtn.style.display = 'inline-block'; // Munculin kalau tadinya hidden
                
                // Efek "Bump" (Membal) di tombol cart
                cartBtn.classList.add('cart-bump');
                setTimeout(() => cartBtn.classList.remove('cart-bump'), 200);

                // 2. Notifikasi Toast Kecil
                ...
                // 3. Balikin Tombol Menu jadi Centang sebentar
                btn.innerHTML = '<i class="bi bi-check-lg"></i> Masuk';
