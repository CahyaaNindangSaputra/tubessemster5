    ...
    <div class="sidebar-right">
        <div>
            <div class="text-center mb-5">
                <h4 class="fw-bold" style="color: var(--primary-color); letter-spacing: -1px; font-size: 1.8rem;">RIEL'S<span style="color: var(--text-dark)">COFFE</span></h4>
                            ...
                            <div class="table-scroll-container border-0 rounded-0">
                                <table class="table table-hover align-middle mb-0">
                                    ...
                                    <tbody id="list-sedang-proses">
                                        ...
                                        <tr id="row-proses-{{ $p->ID_PESANAN }}">
                                            ...
                                            <td>
                                                <div class="d-flex gap-2 align-items-center justify-content-end">
                                                    <form action="{{ route('pesanan.updateStatus', $p->ID_PESANAN) }}" method="POST" class="ajax-form-status" data-id="{{ $p->ID_PESANAN }}">
                                                        @csrf @method('PATCH')
                                                        @if($p->STATUS_PESANAN == 'Proses') <button type="submit" class="btn btn-sm btn-outline-warning rounded-pill btn-action px-3"><i class="bi bi-play-fill me-1"></i> Masak</button>
                                                        @elseif($p->STATUS_PESANAN == 'Dimasak') <button type="submit" class="btn btn-sm btn-outline-info rounded-pill btn-action px-3"><i class="bi bi-bell-fill me-1"></i> Panggil</button>
                                                        @elseif($p->STATUS_PESANAN == 'Siap') <button type="submit" class="btn btn-sm btn-outline-success rounded-pill btn-action px-3"><i class="bi bi-check-all me-1"></i> Selesai</button> @endif
                                                    </form>
                                        ...
                                        @empty
                                        <tr><td colspan="4" class="text-center py-5 text-muted small">Dapur santai.</td></tr>
                                        @endforelse
                                    </tbody>
                                </table>
                            </div>
                        ...
                        <div class="card-modern h-100 d-flex flex-column">
                            ...
                            <div class="p-3 border-top bg-white mt-auto">
                                <div class="d-flex justify-content-between align-items-center bg-success-subtle p-4 rounded-4 shadow-sm border border-success-subtle">
                                    <div>
                                        <small class="text-success fw-bold text-uppercase d-block mb-1">TOTAL OMSET</small>
                                        <div class="badge bg-success rounded-pill px-3 py-1 fs-6">{{ \Carbon\Carbon::parse($tanggalFilter)->format('d M Y') }}</div>
                                    </div>
            ...
            <div class="tab-pane fade" id="nav-menu">
                <div class="d-flex justify-content-between align-items-center mb-5">
                    <div>
                        ...
                        <p class="text-muted mb-0 fs-5">Kelola ketersediaan menu.</p>
                    </div>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function loadDashboardData() {
                ...
                fetch(window.location.href).then(response => response.text()).then(html => {
                    ...
                    // Update Modals Container (INI YANG BIKIN MODAL BARU MUNCUL)
                    let newModals = doc.getElementById('modals-container').innerHTML;
                    if(document.getElementById('modals-container').innerHTML !== newModals) { document.getElementById('modals-container').innerHTML = newModals; }

                    let newMenunggu = doc.getElementById('list-menunggu-bayar').innerHTML;
                    if(document.getElementById('list-menunggu-bayar').innerHTML !== newMenunggu) { document.getElementById('list-menunggu-bayar').innerHTML = newMenunggu; bindActions(); }

                    let newProses = doc.getElementById('list-sedang-proses').innerHTML;
                    if(document.getElementById('list-sedang-proses').innerHTML !== newProses) { document.getElementById('list-sedang-proses').innerHTML = newProses; bindActions(); }

                    let newRiwayat = doc.querySelector('#nav-pos .col-lg-5 .table tbody').innerHTML;
                    if(document.querySelector('#nav-pos .col-lg-5 .table tbody').innerHTML !== newRiwayat) { document.querySelector('#nav-pos .col-lg-5 .table tbody').innerHTML = newRiwayat; }

                    let newStats = doc.querySelector('.row.mb-5.g-4').innerHTML;
                    if(document.querySelector('.row.mb-5.g-4').innerHTML !== newStats) { document.querySelector('.row.mb-5.g-4').innerHTML = newStats; }
